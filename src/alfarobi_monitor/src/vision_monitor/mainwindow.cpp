...
MainWindow::MainWindow(QWidget *parent) :
    ...
    icon_path << ros::package::getPath("alfarobi_monitor") << "/icon/vision_monitor/logo_alfarobi.png";
    QIcon icon_file;
    icon_file.addFile(icon_path.str().c_str());
    this->setWindowIcon(icon_file);

    std::stringstream style_path;
    style_path << ros::package::getPath("alfarobi_monitor") << "/style/vision_style.qss";
    ...
    LUT_pub_ = nh_.advertise<alfarobi_msgs_srvs_actions::LUT>("LUT_data",10);
...
void MainWindow::browseLUT(){
    QString LUT_path = QFileDialog::getOpenFileName(this,"Search LUT file...",ros::package::getPath("alfarobi_vision").c_str(),"XML File(*.xml)");
...
void MainWindow::loadBallConfig(){
    ...
    try{
        config_file = YAML::LoadFile(ros::package::getPath("alfarobi_vision") + "/config/v9_ball_detector/saved_config.yaml");
...
//     YAML::Node config_file;
//     try{
//         config_file = YAML::LoadFile(ros::package::getPath("alfarobi_vision") + "/config/v10_goalpost_detector/saved_config.yaml");
//     }catch(const std::exception &e){
//         ROS_ERROR("[vision_monitor] Problem while opening config file !!!");
//     }
