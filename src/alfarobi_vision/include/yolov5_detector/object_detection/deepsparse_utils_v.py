...
def annotate_image(
    ...
    for idx in range(boxes.shape[0]): #in this loop, change idx to 0 to make it only detect 1 object
        ...
        if scores[idx] > score_threshold:
            ...
            program.publishPos(centerx,centery,-1)
            
    
    # img_res=np.zeros()

    # exeption handling for sudden stop from img_source
    if program.img_sub.get_num_connections() == 0:
        img_res = numpy.zeros((418,418,3), dtype=numpy.uint8)

        if program.stop_time == None:
            program.stop_time = time.time()
        elif time.time() > program.stop_time + 5:
            program.suddenStop()
    else:
