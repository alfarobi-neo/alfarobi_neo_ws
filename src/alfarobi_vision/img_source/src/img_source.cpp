...
ImageSource::ImageSource()
    :nh_(ros::this_node::getName()),
        ...
        LUT_dir = ros::package::getPath("img_source") + "/LUT/tabel_warna.xml";
...
void ImageSource::imageCallback(const sensor_msgs::ImageConstPtr &_msg){
    ...
    try{
        ...
        ROS_ERROR("[image_source] cv bridge exception: %s",e.what());
        return;
    }
...
void ImageSource::lutCallback(const vision_utils::LUTConstPtr &_msg){
...
std::pair<cv::Mat, vision_utils::FieldBoundary > ImageSource::getFieldImage(const cv::Mat &_segmented_green){
