...
Localization::Localization():nh_(ros::this_node::getName()),it_(nh_),
    sw_sub_(nh_, "/img_source_node/segment_white",1),
    isg_sub_(nh_,"/img_source_node/inv_segment_green",1),
    fb_sub_(nh_,"/img_source_node/field_boundary",1),
    ...
    particles_state_pub_(nh_.advertise<alfarobi_msgs_srvs_actions::Particles> ("particles_state", 10)),
    features_pub_(nh_.advertise<alfarobi_msgs_srvs_actions::Features > ("field_features", 10)),
    ...
    line_tip_pub_(nh_.advertise<alfarobi_msgs_srvs_actions::LineTip> ("line_tip", 10)),
    #ifdef GAZEBO
...
void Localization::utilsCallback(const sensor_msgs::ImageConstPtr &_sw_msg,
                                 ...
                                 const alfarobi_msgs_srvs_actions::FieldBoundary::ConstPtr &_fb_msg,
...
void Localization::publishTipPoints(alfarobi_msgs_srvs_actions::LineTip _tip_points){
...
void Localization::measurementModel(Particles &_particles_state,
                                    alfarobi_msgs_srvs_actions::Features _features_arg,
                                    float &_weight_avg){
...
//        }
    ...
    alfarobi_msgs_srvs_actions::Features _features = _features_arg;
    for(std::vector<alfarobi_msgs_srvs_actions::Feature>::iterator it = _features.feature.begin();
...
//                                 false,false,false,false,false,false};
        ...
        float c_t = cos(tetha);
        float s_t = sin(tetha);
        for(std::pair<std::vector<alfarobi_msgs_srvs_actions::Feature>::iterator,
...
void Localization::getFeaturesModels(Points &_target_points,
                                 ...
                                 alfarobi_msgs_srvs_actions::Features &_features_arg){
    alfarobi_msgs_srvs_actions::Features _features;
...
//        alfarobi_msgs_srvs_actions::Feature feature_data;
...
//    cv::waitKey(0);
    ...
    // for(alfarobi_msgs_srvs_actions::Feature &feature_data:_features.feature){
    //     cv::line(visualize,cv::Point(feature_data.param1+remap_origin.x, feature_data.param2+remap_origin.y),
    //             cv::Point(feature_data.param3+remap_origin.x, feature_data.param4+remap_origin.y),
    //             cv::Scalar(255,0,255),2);
    // }
...
void Localization::getFeaturesModels(Points &_target_points,
                                 Vectors3 &_line_models,
                                 alfarobi_msgs_srvs_actions::Features &_features, alfarobi_msgs_srvs_actions::LineTip &_line_tip){
...
alfarobi_msgs_srvs_actions::Features Localization::getLineIntersection(Vectors3 _line_models){
    // Not used yet
    alfarobi_msgs_srvs_actions::Features features;
...
//            int features_type = getIntersectionType(segmented_white_, intersect_line, cv::Point(x.at<float > (0), x.at<float > (1)));
            ...
            switch(features_type){
                case 2:
                putText(image_in_, "L", cv::Point(x.at<float>(0), x.at<float>(1)), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(25,25,0), 2);
                circle(image_in_, cv::Point(x.at<float>(0), x.at<float>(1)), 4, cv::Scalar(0,255,0), cv::FILLED);
//                features.push_back(line_feature);
                break;
                case 3:
                putText(image_in_, "T", cv::Point(x.at<float>(0), x.at<float>(1)), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(25,25,0), 2);
                circle(image_in_, cv::Point(x.at<float>(0), x.at<float>(1)), 4, cv::Scalar(0,255,0), cv::FILLED);
//                features.push_back(line_feature);
                break;
                case 4:
                putText(image_in_, "X", cv::Point(x.at<float>(0), x.at<float>(1)), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(25,25,0), 2);
                circle(image_in_, cv::Point(x.at<float>(0), x.at<float>(1)), 4, cv::Scalar(0,255,0), cv::FILLED);
//                features.push_back(line_feature);
                break;
                default:
...
//        cv::circle(draw,cv::Point(,),1,cv::Scalar(255),cv::FILLED);
    }
#ifdef DEBUG
    if(!ball){
...
void Localization::publishData(){
    alfarobi_msgs_srvs_actions::Particles particles_msg;
    ...
    if(ball_pos_.x != -1.0 &&
        ...
        // ball_pos = fastPointsProjection(ball_pos, true);
        ...
        if(ball_pos.size() > 0){
            ...
            ball_pos_msg.x = ball_pos.front().y - 10;
...
//    alfarobi_msgs_srvs_actions::LineTip tip_points;
