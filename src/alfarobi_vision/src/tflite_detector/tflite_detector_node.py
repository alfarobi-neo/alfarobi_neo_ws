...
class VisionLoader:
	def __init__(self):   
		...
		self.ball_dist_pub = rospy.Publisher("ball_dist", Int32, queue_size=100)
	...
	def publishDist(self, d:Int32):
		self.ball_dist_pub.publish(int(d))
...
for frame in main:
	...
	for i in range(len(scores)):
		if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
			...
			cv2.putText(frame1,'Distance: ',(30,100),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
			cv2.putText(frame1,str(int(d)),(200,100),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
