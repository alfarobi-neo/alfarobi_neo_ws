#include <iomanip>
#include <stdexcept>
#include "quintic_walk/utils/Spline.hpp"
...
void Spline::addPart(const Polynom& poly, double min, double max)
{
...
double Spline::interpolation(double x, double (Polynom::*func)(double) const) const
{
  // Empty case
  if (_splines.size() == 0)
  {
...
double Spline::interpolationMod(double x, double (Polynom::*func)(double) const) const
{
  if (x < 0.0)
  {
    x = 1.0 + (x - ((int)x / 1));
