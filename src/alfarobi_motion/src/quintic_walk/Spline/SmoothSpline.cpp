...
{
  _points.push_back({ time, position, velocity, acceleration });
  computeSplines();
}
...
void SmoothSpline::computeSplines()
{
  ...
  std::sort(_points.begin(), _points.end(), [](const Point& p1, const Point& p2) -> bool { return p1.time < p2.time; });
  ...
  for (size_t i = 1; i < _points.size(); i++)
  {
    ...
    if (time > 0.00001)
    {
      Spline::_splines.push_back(
          { polynomFit(time, _points[i - 1].position, _points[i - 1].velocity, _points[i - 1].acceleration,
...
void SmoothSpline::importCallBack()
{
  ...
  _points.push_back({ tBegin, Spline::pos(tBegin), Spline::vel(tBegin), Spline::acc(tBegin) });
  ...
  for (size_t i = 1; i < size; i++)
  {
    ...
    if (fabs(t2 - t1) < 0.0001 && fabs(pos2 - pos1) < 0.0001 && fabs(vel2 - vel1) < 0.0001 &&
  ...
  _points.push_back({ tEnd, Spline::pos(tEnd), Spline::vel(tEnd), Spline::acc(tEnd) });
}

Polynom SmoothSpline::polynomFit(double t, double pos1, double vel1, double acc1, double pos2, double vel2,
                                 double acc2) const
{
  if (t <= 0.00001)
  {
